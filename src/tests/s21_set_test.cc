...
class SetTest : public ::testing::Test {
 protected:
  s21::Set<int> s21_empty;
  std::set<int> std_empty;
  s21::Set<std::string> s21_string{"test1", "test2", "test1", "test2", "test0"};
  std::set<std::string> std_string{"test1", "test2", "test1", "test2", "test0"};
  s21::Set<int> s21_int{3, 3, 2, 5, 7, 3, 1, 3};
  std::set<int> std_int{3, 3, 2, 5, 7, 3, 1, 3};
  const s21::Set<char> s21_const{'s', 'd', 's', 'a', 's', 'a', 'z', 'q'};
  const std::set<char> std_const{'s', 'd', 's', 'a', 's', 'a', 'z', 'q'};
};
...
TEST_F(SetTest, testConstructor4) {
...
TEST_F(SetTest, testCopy) {
  s21::Set<int> s21_copy1(s21_int);
  s21::Set<int> s21_copy2(s21_empty);
...
TEST_F(SetTest, testMove) {
  s21::Set<std::string> s21_move1(std::move(s21_string));
  s21::Set<int> s21_move2(std::move(s21_empty));
  std::set<std::string> std_move1(std::move(std_string));
  std::set<int> std_move2(std::move(std_empty));
...
TEST_F(SetTest, testCopyOperator1) {
  s21::Set<std::string> s21_copy = s21_string;
...
TEST_F(SetTest, testCopyOperator2) {
  s21::Set<int> s21_copy{1, 2, 3, 3, 2, 1, 10, 987, 987};
  s21_copy = s21_empty;
...
TEST_F(SetTest, testCopyOperator3) {
  s21_empty = s21_int;
  std_empty = std_int;
...
TEST_F(SetTest, testMoveOperator1) {
  s21::Set<int> s21_move{1, 2, 3, 3, 2, 1, 10, 987, 987};
  s21_move = std::move(s21_empty);

  EXPECT_TRUE(s21_move.empty());
  EXPECT_FALSE(s21_empty.empty());
  EXPECT_TRUE(s21_move.begin() == s21_move.end());
  EXPECT_EQ(s21_empty.size(), 5);
}

TEST_F(SetTest, testMoveOperator2) {
...
TEST_F(SetTest, testInsert1) {
  auto s21_it = s21_empty.insert(10);
  auto s21_it2 = s21_empty.insert(20);
  auto s21_it3 = s21_empty.insert(10);

  auto std_it = std_empty.insert(10);
  auto std_it2 = std_empty.insert(20);

  EXPECT_EQ(*s21_it.first, *std_it.first);
  EXPECT_EQ(*s21_it2.first, *std_it2.first);
...
TEST_F(SetTest, testErase) {
  ...
  for (; s21_checkIt != s21_string.end(); ++s21_checkIt) {
    EXPECT_EQ(*s21_checkIt, *std_checkIt);
    ++std_checkIt;
  }
}

TEST_F(SetTest, testSwap1) {
  s21::Set<int> s21_swap = {5, 15, 25};
  std::set<int> std_swap = {5, 15, 25};
...
TEST_F(SetTest, testMerge1) {
  s21::Set<std::string> s21_merge = {"test0", "merge2", "merge2",
                                     "test1", "merge3", "test2"};
...
TEST_F(SetTest, testMerge2) {
  s21::Set<int> s21_test = {3, 2, 1, 6, 7};
  s21::Set<int> s21_merge = {8, 13, 11, 16, 14};
...
TEST_F(SetTest, testFind2) {
  ...
  EXPECT_EQ(*s21_const.find('s'), *std_const.find('s'));
  EXPECT_EQ(*s21_const.find('w'), '\0');
}
